...
//去掉tree前面的小图标 wc 20140207
cc.push("<span class=\"tree-icon tree-folder "+(item.iconCls?item.iconCls:"")+"\""+(item.iconCls=='icon-none'?" style='display:none'":'')+"></span>");
//end
...
//去掉tree前面的小图标 wc 20140207
cc.push("<span class=\"tree-icon tree-folder tree-folder-open "+(item.iconCls?item.iconCls:"")+"\""+(item.iconCls=='icon-none'?" style='display:none'":'')+"></span>");
//end
//cc.push("<span class=\"tree-icon tree-folder tree-folder-open "+(item.iconCls?item.iconCls:"")+"\"></span>");
}else{
cc.push("<span class=\"tree-indent\"></span>");
//去掉tree前面的小图标 wc 20140207
cc.push("<span class=\"tree-icon tree-file "+(item.iconCls?item.iconCls:"")+"\""+(item.iconCls=='icon-none'?" style='display:none'":'')+"></span>");
//end
...
if(_atree) {
	cc.push('<a class="pagination-load" title="刷新当前树" href="javascript:void(0)" onclick="reloadTree(this)" style="text-decoration: none;">&nbsp;&nbsp;&nbsp;&nbsp;</a>');
	_atree = false;
}
cc.push("</div>");
if(item.children&&item.children.length){
...
//----增加树加载效果--------------------------------------start modify wc 2014/1/22
var curobj = $(this);
var rand = Math.floor(Math.random() * (100 - 1) + 1);
var datagrid_mask_class = "datagrid-mask-tree-"+rand;
var datagrid_mask_msg_class = "datagrid-mask-msg-tree-"+rand;
curobj.after('<div class=\"'+datagrid_mask_class+' new_datagrid-mask\" style=\"display:block\"></div>');
var msg = $("<div class=\""+datagrid_mask_class+" new_datagrid-mask-msg\" style=\"display:block;left:50%\"></div>").html('请稍待...').insertAfter(this);
msg._outerHeight(40);
msg.css({
...
$.ajax({type:opts.method,url:opts.url,data:_19d,dataType:"json",success:function(data){
	//----增加树加载效果--------------------------------------start modify wc 2014/1/22
	curobj.siblings("div."+datagrid_mask_class).remove();
	curobj.siblings("div."+datagrid_mask_msg_class).remove();
	//---增加树加载效果---- end
	
					//------------------------------------------start modify mhj 2014/1/21
					var tdata = data;
					var checked = "";
					var iconCls = opts.iconCls;
					var _valueField = opts.valueField ? opts.valueField:'id';

					if(opts.value!=null){	//选中的参数，由data-options传递过来，由逗号分隔字符串
						checked = opts.value.split(",");
					}

					function addCheck(data){
						for(var i=0;i<data.length;i++){
							for(var k=0;k<checked.length;k++){
								if(data[i][_valueField] == checked[k]){
									data[i].checked = true;
								}
							}
							if(iconCls=="none"){
								data[i].iconCls = "icon-none";
							}
							if(data[i].children!=null){
								addCheck(data[i].children);
							}
						}
					}
					addCheck(tdata);
...
var _278="<div class='DmsgBox'>"+msg+"</div>";
switch(icon){
case "error":
...
var _27b="<div class=\"messager-icon messager-question\"></div>"+"<div class='DmsgBox'>"+msg+"</div>"+"<div style=\"clear:both;\"/>";
var _27c={};
...
$.fn.tabs.defaults={width:"auto",height:"auto",headerWidth:150,tabWidth:"auto",tabHeight:32,selected:0,showHeader:true,plain:false,fit:false,border:true,tools:null,toolPosition:"right",tabPosition:"top",scrollIncrement:100,scrollDuration:400,onLoad:function(_345){
...
// --------------修改表单ajax验证返回值处理 wc 20140126
if (typeof _430 == 'string') {
	if(_430=='true') return true;
	if(_430.indexOf('{') === 0) {
		_430 = eval('('+_430+')');
	} else {
		return false;
...
return _430.status=='success'?true:false; 
// ----------------end
//return _430=="true";
...
$(_68a)[0].innerHTML = _68e.join(""); //20140114 wc 优化修改
//$(_68a).html(_68e.join("")); //原版
...
$.ajax({type:opts.method,url:opts.url,data:_6c5,dataType:"json",success:function(data){
	//wc 20140116 根据特殊格式做修改
	if(data.ccsdata){
		data.rows = data.ccsdata;
	} //end
_6c6(data);
},error:function(){
